...
public class method연습 {
	...
	public static void main(String[] args) {
		 ...
		 * 		-정수형(문자) : char(정수형, ASCII코드 문자표에 저장되어 있는 코드로 저장) 1글자
		 * 		-불리언 : boolean(true, false)
		 * 
		 * 	-참조변수 : 객체의 주소를 저장하는 변수(객체명) : new 키워드를 통해 객체 생성
		 * 		-힙영역에 객체가 생성, 스택영역에 객체의 주소를 연결
		 * 		-기본형(일반변수)를 제외한 모든 형태는 참조변수
		 * 
		 * 	-자료형변환 : 변수, 상수, 리터럴 값들을 일시적으로 다른 자료형으로 변환
		 * 		-자료형변현 : 정수 -> 실수, 작은공간 -> 큰공간
		 * 		-명시적현변환 : 자동형변환이 안되는 형태	(자료형)변수명
		 * 
		 * 
		 * 
		 * 2. 연산자
		 ...
		 *
		 *	-getter / setter메서드를 만들어 private의 멘버변수에 접근
		 *
