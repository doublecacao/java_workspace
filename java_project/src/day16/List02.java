...
class FoodManager{
	...
	//그외 필요한 멤버변수는 선택
	private List<String> menu = new ArrayList<String>();			//메뉴를 추가받는 리스트배열 생성
	private List<Integer> price = new ArrayList<Integer>();			//메뉴의 가격을 추가받는 리스트배열 생성
	
	private List<String> menuchoice = new ArrayList<String>();		//선택한 메뉴의 번호를 저장받는 리스트배열 생성
	private List<Integer> pricechoice = new ArrayList<Integer>();	//선택한 메뉴의 가격을 저장받는 리스트배열 생성
	
	private List<Integer> saveNum = new ArrayList<Integer>();		//선택한 메뉴의 갯수를 저장받는 리스트배열 생성
	private int cnt=0;												//프로그램 종료를 위한 변수
	...
	public void printMenu() {								//추가한 메뉴를 메뉴판에 출력하는 메소드
		...
		System.out.println((i+1)+".프로그램종료");				//마지막 메뉴의 다음 번호는 프로그램종료 출력
	}
	...
	public void menuOrder() {								//메뉴를 주문받는 메소드
		...
		do {
			...
			if(choiceMenu==(menu.size()+1)) {				//프로그램 종료 선택시 메뉴판 종료
				cnt=1;
